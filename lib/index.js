...
function validateProvider(route, config) {
	...
	for (const [remoteId, reference] of Object.entries(config.modelMappings)) {
		...
		if (!OFFICIAL_REFS.has(`${reference.provider}/${reference.id}`)) throw new Error(`Relay provider "${route}" maps "${remoteId}" to unknown model ${reference.provider}/${reference.id}`);
	}

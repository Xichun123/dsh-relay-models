...
function protocolLabel(protocol: RelayProtocol): string {
  if (protocol === 'anthropic-messages') return 'Anthropic Messages'
  if (protocol === 'openai-responses') return 'OpenAI Responses'
  return 'OpenAI Chat Completions'
}

function officialModel(reference: { provider: string; id: string }): OfficialModelSummary | undefined {
...
function MappingEditor({
  ...
  const results = useMemo(() => {
    if (!query.trim()) {
      return status.candidates.map(candidate => officialModel(candidate)).filter((model): model is OfficialModelSummary => model !== undefined)
    }
    const needle = query.trim().toLowerCase()
    return OFFICIAL_MODELS.filter(model =>
...
function ProviderCard({
  ...
  const statuses = useMemo(() => relayModelStatuses(provider, OFFICIAL_MODELS), [provider])
            ...
            {statuses.map(status => {
              ...
              return [
                <tr key={status.id}>
                  ...
                  <td>{source ? <span>{source.provider}/{source.id}{mapping ? <span className="drm-pill">人工</span> : null}</span> : <span className="drm-muted">未匹配</span>}</td>
                ...
                mappingModel === status.id ? <tr key={`${status.id}:mapping`}><td colSpan={4}><MappingEditor status={status} onClose={() => { setMappingModel(undefined) }} onMap={async model => {
...
export function RelayModelsSection({ api }: Props): ReactNode {
  ...
  return (
    <section className="drm-section">
      <div className="drm-head"><div className="drm-head-copy"><h2 className="drm-title">中转模型</h2><p className="drm-muted">自动发现中转站模型，匹配 pi 官方元数据，并在同一 Provider 内按模型选择 OpenAI Chat Completions、OpenAI Responses 或 Anthropic Messages。此插件不会向 Agent 注册工具，也不会添加任何上下文。</p></div>{snapshot ? <AddProvider api={api} snapshot={snapshot} onDone={reload} /> : null}</div>
      {error ? <div className="drm-error">{error}<div><button className="drm-button" onClick={() => { void reload() }}>重试</button></div></div> : null}
      {snapshot && Object.keys(snapshot.config.providers).length === 0 ? <div className="drm-empty">还没有中转站。点击“添加中转站”开始。</div> : null}
      {snapshot ? Object.entries(snapshot.config.providers).map(([route, provider]) => <ProviderCard key={route} route={route} provider={provider} snapshot={snapshot} api={api} onReload={reload} />) : null}
    </section>
  )
}
